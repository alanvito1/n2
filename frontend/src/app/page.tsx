    ...
    <main className="min-h-screen p-8 bg-gray-50 flex flex-col items-center">
      <div className="max-w-4xl w-full">
        <header className="mb-10 flex justify-between items-center">
            <h1 className="text-3xl font-bold text-gray-900">N2 Secure Audit</h1>
        ...
        <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-200">
          <h2 className="text-xl font-semibold mb-6 text-gray-800">Upload Confidential Document</h2>

          <div className="mb-6">
            <label className="block text-sm font-medium text-gray-700 mb-2">Select file for the Vault</label>
            <input
              type="file"
          ...
          >
            {status === 'IDLE' ? 'Start Audit' : 'Processing...'}
          </button>
             ...
             <div className="mt-8 p-6 bg-gray-50 rounded-md border border-gray-200">
                ...
                <div className="space-y-4">
                    <div className="flex items-center">
                        ...
                        <span className="text-sm text-gray-700">1. Upload to Secure Enclave</span>
                    </div>
                    <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-3 ${status === 'PROCESSING' || status === 'COMPLETED' ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        <span className="text-sm text-gray-700">2. Extraction via AI & OCR (TEE)</span>
                    </div>
                    <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-3 ${status === 'COMPLETED' ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        <span className="text-sm text-gray-700">3. On-Chain Attestation (TON)</span>
                    </div>
                </div>
             </div>
          )}
          ...
          {status === 'COMPLETED' && (
              <div className="mt-6 p-6 bg-green-50 border border-green-200 rounded-md">
                 <h3 className="text-lg font-medium text-green-800 mb-2">Audit Completed Successfully</h3>
                 <div className="text-sm text-green-700 space-y-2 break-all">
                     <p><strong>Report Hash:</strong> {resultHash}</p>
